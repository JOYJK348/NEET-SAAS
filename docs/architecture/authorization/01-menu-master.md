...
3. **Three-Level Max Depth**: Navigation depth is strictly limited to 3 levels to maintain mobile-friendliness and prevent UI layout distortion:
   * Level 1: Module Group (e.g., *Learning*)
   * Level 2: Sub-module / Entity (e.g., *Study Materials*)
   * Level 3: Auxiliary action page / Detail view (e.g., *Upload Vault*)
...
* **Breadcrumb Path**: Determined dynamically by parsing upward from the active child node to the root.
  * *Example Path:* `Learning` ➔ `Study Materials` (derived from `/learning/materials`).
...
* When introducing a new functional module (e.g., *Hostel Management*):
