...
Unlike authorization (which validates _who_ can do an action), the **Policy Engine** evaluates **business constraints and rules** (_how_ actions must behave based on environment context, values, or metrics).
