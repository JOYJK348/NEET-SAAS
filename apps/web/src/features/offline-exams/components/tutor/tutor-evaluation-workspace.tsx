...
export function TutorEvaluationWorkspace({ examId, submissionId }: TutorEvaluationWorkspaceProps) {
  ...
  if (isLoading) {
    return <div className="py-24 text-center text-slate-400 font-medium">Loading evaluation workspace...</div>;
  }

  if (!detail) {
    return <div className="py-24 text-center text-slate-400 font-medium">Submission not found.</div>;
  }
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50 text-slate-800 overflow-hidden w-full">
      {/* Top Navbar */}
      <div className="flex items-center justify-between px-6 py-3 border-b border-slate-200 bg-white shrink-0 shadow-sm">
        <div className="flex items-center gap-4">
          <Link
            ...
            className="inline-flex items-center gap-2 px-3 py-1.5 rounded-xl bg-slate-100 border border-slate-200 text-xs font-bold text-slate-700 hover:text-slate-900 hover:bg-slate-200 transition"
          >
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-[60%] border-r border-slate-200 flex flex-col bg-slate-100/60">
          {/* PDF Toolbar */}
          <div className="px-4 py-2 bg-white border-b border-slate-200 flex items-center justify-between text-xs shrink-0 shadow-sm">
        ...
        <div className="w-[40%] flex flex-col bg-white overflow-y-auto p-6 space-y-6">
          {/* Returned Alert Banner */}
          {detail.evaluationStatus === 'RE_EVALUATION' && (
            <div className="p-4 bg-rose-50 border border-rose-200 rounded-2xl text-rose-800 text-xs space-y-1.5 shadow-sm">
              <div className="flex items-center gap-2 font-bold text-rose-700">
                <RotateCcw className="w-4 h-4 text-rose-600" /> Returned by Tenant Admin for
                Re-Evaluation
              </div>
              <p className="italic text-rose-800">"{detail.rejectionReason}"</p>
            </div>
          )}

          {/* Read Only Banner */}
          {isReadOnly && (
            <div className="p-4 bg-purple-50 border border-purple-200 rounded-2xl text-purple-800 text-xs flex items-center gap-3 shadow-sm">
              <Lock className="w-5 h-5 text-purple-600 shrink-0" />
              <div>
                <p className="font-bold text-purple-900">Evaluation Phase Locked</p>
          ...
          <div className="bg-slate-50/80 p-4 rounded-2xl border border-slate-200/60 space-y-2 text-xs">
            <h4 className="font-bold text-slate-900 text-sm flex items-center gap-2">
              <User className="w-4 h-4 text-indigo-600" /> Student Profile
            </h4>
            <div className="grid grid-cols-2 gap-2 text-slate-500 pt-1 border-t border-slate-200/60">
              <p>
                Name: <span className="text-slate-800 font-semibold">{detail.studentName}</span>
              </p>
              <p>
                Admission No:{' '}
                <span className="text-slate-800 font-mono font-medium">{detail.studentAdmissionId}</span>
              </p>
              <p>
                Submitted:{' '}
          ...
          <div className="bg-slate-50/80 p-5 rounded-2xl border border-slate-200/60 space-y-4">
            <div className="flex items-center justify-between border-b border-slate-200/60 pb-3">
              <h4 className="font-bold text-slate-900 text-sm">Dynamic Section Marks Entry</h4>
              <div className="text-right">
                <span className="text-xs text-slate-500 font-medium">Total Calculated:</span>
                <span className="text-xl font-extrabold text-teal-700 ml-2 font-mono">
                  {calculatedTotalMarks}
                </span>
                <span className="text-xs text-slate-400 font-normal"> / {detail.totalMarks}</span>
              </div>
            </div>
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      className="w-24 bg-slate-50 border border-slate-200 rounded-lg px-3 py-1.5 text-sm font-bold text-slate-900 text-right font-mono focus:outline-none focus:border-indigo-500 disabled:opacity-50"
                    />
          ...
          <div className="space-y-2">
            ...
            <textarea
              ...
              className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs text-slate-900 focus:outline-none focus:border-indigo-500 disabled:opacity-50 font-medium"
            />
          </div>
          ...
          {detail.history && detail.history.length > 0 && (
            <div className="bg-slate-50/80 p-4 rounded-2xl border border-slate-200/60 space-y-3">
              <h4 className="text-xs font-bold text-slate-700 flex items-center gap-2 uppercase tracking-wider">
                <History className="w-4 h-4 text-indigo-600" /> Evaluation Revision History
              </h4>
              <div className="space-y-2 max-h-36 overflow-y-auto">
                {detail.history.map((h) => (
                  <div
                    key={h.id}
                    className="bg-white p-2.5 rounded-lg border border-slate-200 text-xs flex items-center justify-between shadow-sm"
                  >
                    <div>
