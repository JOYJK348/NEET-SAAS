...
import { MoreHorizontal, Calendar, MapPin, Users, BookOpen } from 'lucide-react';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export function BatchCard({
  ...
  return (
    <Card
      className="w-full rounded-2xl border border-slate-200 bg-white shadow-2xs hover:shadow-md hover:border-blue-300 transition-all duration-200 overflow-hidden"
      ...
      <CardContent className="p-4 sm:p-5 space-y-4">
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 space-y-1">
            <span className="inline-flex items-center px-2.5 py-0.5 rounded-md text-[11px] font-mono font-extrabold bg-blue-50 text-[#0052CC] border border-blue-200">
              {batch.code}
            </span>
            <h3
              className="font-extrabold text-[#0B2447] text-base leading-snug truncate hover:text-[#0052CC] cursor-pointer"
        ...
        <div className="grid grid-cols-2 gap-2 text-xs">
          <div className="flex items-center gap-1.5 p-2 bg-slate-50 rounded-xl border border-slate-200">
            <BookOpen className="h-3.5 w-3.5 text-[#0052CC] shrink-0" />
            <span className="truncate font-extrabold text-[#0B2447]">{batch.courseName}</span>
          </div>
          <div className="flex items-center gap-1.5 p-2 bg-slate-50 rounded-xl border border-slate-200">
            <MapPin className="h-3.5 w-3.5 text-[#0052CC] shrink-0" />
            <span className="truncate font-bold text-slate-700">{batch.branchName}</span>
          </div>
        </div>
        ...
        <div className="pt-3 border-t border-slate-100 flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 w-full">
            <Button
              variant="outline"
              size="sm"
              className="flex-1 rounded-xl border-slate-200 hover:border-blue-300 hover:bg-blue-50 text-[#0052CC] font-extrabold text-xs h-9"
              onClick={() => onView?.(batch.id)}
            >
              View Batch
            </Button>
            <Button
              variant="outline"
              size="sm"
              className="flex-1 rounded-xl border-slate-200 hover:border-blue-300 hover:bg-slate-50 text-slate-700 font-bold text-xs h-9"
          ...
          {(VALID_TRANSITIONS[batch.status]?.length ?? 0) > 0 && (
            <DropdownMenu>
              ...
              <DropdownMenuContent
                align="end"
                className="w-48 rounded-xl shadow-lg border-slate-200 bg-white"
              >
                <DropdownMenuItem
                  onClick={() => onView?.(batch.id)}
                ...
                {VALID_TRANSITIONS[batch.status]?.map((targetStatus) => (
                  <DropdownMenuItem
                    ...
                    className={cn(
                      ...
                      batch.status === targetStatus ? 'text-[#0052CC]' : '',
                    )}
                  >
